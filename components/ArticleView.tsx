...
export const ArticleView: React.FC<ArticleViewProps> = ({ post, onBack, onAuthorClick }) => {
  ...
  return (
    <div className="min-h-screen bg-white animate-in fade-in duration-300 pb-24 md:pb-0">
      {/* Article Header / Nav */}
      <div className="sticky top-0 bg-white/90 backdrop-blur border-b border-slate-100 z-20 px-4 py-3 flex items-center justify-between max-w-4xl mx-auto w-full">
        <button 
          onClick={onBack}
        ...
        <div className="flex gap-2">
            <button className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-700"><Bookmark className="w-4 h-4" /></button>
            <button className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-700"><Share className="w-4 h-4" /></button>
        </div>
      </div>
        ...
        <div className="lg:col-span-8">
          ...
          <div className="mb-8">
             ...
             <div 
               ...
               className="flex items-center gap-4 py-5 border-y border-slate-100 cursor-pointer hover:bg-slate-50 transition-all rounded-lg px-3 -mx-3 group"
             >
                <img src={post.author.avatarUrl} alt="" className="w-14 h-14 rounded-full border border-slate-200 shadow-sm" />
                <div className="flex-1 min-w-0">
                   <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold text-lg text-slate-900 group-hover:text-indigo-600 transition-colors">{post.author.name}</p>
                   ...
                   <div className="flex flex-wrap items-center gap-3 mt-1">
                     ...
                     <span className="text-sm text-slate-500">
                       Nitor Score: <span className="font-mono text-indigo-600 font-bold">{post.author.nitorScore}</span>
                     </span>
                   </div>
                </div>
                
                <button className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-slate-200 text-slate-600 text-xs font-bold group-hover:border-indigo-200 group-hover:text-indigo-600 transition-colors shadow-sm">
          ...
          {/* Abstract */}
          {post.abstract && (
            <div className="bg-slate-50 p-8 rounded-2xl mb-10 border border-slate-100">
          ...
          {/* Body */}
          <article className="prose prose-lg prose-slate prose-headings:font-serif prose-p:font-serif prose-p:text-lg prose-p:leading-loose prose-li:font-serif text-slate-800 max-w-none">
             ...
             <div className="mt-12 pt-8 border-t border-slate-100">
               <div className="flex flex-wrap gap-2">
                  {post.keywords.map((k, i) => (
                    <span key={i} className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-sm hover:bg-slate-200 cursor-pointer transition-colors">
                      #{k}
                    </span>
                  ))}
               </div>
          ...
          {/* Interaction Layer */}
          <CommentSection comments={post.commentsList || []} currentUser={CURRENT_USER} />
        </div>
        ...
        <div className="hidden lg:block lg:col-span-4 space-y-8">
           <div className="sticky top-24">
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                 <h4 className="font-bold text-slate-900 mb-4">Impact & Citations</h4>
